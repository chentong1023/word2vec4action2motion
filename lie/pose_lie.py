...
class LieSkeleton(object):
    def __init__(self, raw_translation, kinematic_tree, tensor):
        ...
        self._raw_translation = (
            self.tensor(raw_translation.shape).copy_(raw_translation).detach()
        )
    ...
    def get_translation_joints(self, joints):
        ...
        for i in range(1, self._raw_translation.shape[0]):
            _translation[:, i, :] = (
                torch.norm(
                    joints[:, i, :] - joints[:, self._parents[i], :], p=2, dim=1
                )[:, None]
                * _translation[:, i, :]
            )
    ...
    def get_translation_bone(self, bonelengths):
        ...
        self._translation = (
            self._raw_translation.clone()
            .detach()
            .expand(bonelengths.size(0), -1, -1)
            .clone()
            .to(bonelengths.device)
        )
    ...
    def inverse_kinemetics(self, joints):
        ...
        root_matR = (
            torch.eye(3, dtype=joints.dtype)
            .expand((joints.shape[0], -1, -1))
            .clone()
            .detach()
            .to(joints.device)
        )
        for chain in self._kinematic_tree:
            ...
            for j in range(len(chain) - 1):
                # (batch, 3)
                u = (
                    self._raw_translation[chain[j + 1]]
                    .expand(joints.shape[0], -1)
                    .clone()
                    .detach()
                    .to(joints.device)
                )
    ...
    def forward_kinematics(
        self, lie_params, joints, root_translation, do_root_R=False, scale_inds=None
    ):
        ...
        for chain in self._kinematic_tree:
            ...
            for i in range(1, len(chain)):
                ...
                joints[:, chain[i], :] = (
                    torch.matmul(matR, translation_vec).squeeze_()
                    + joints[:, chain[i - 1], :]
                )
        return joints
